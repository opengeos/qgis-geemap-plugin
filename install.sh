#!/bin/bash
# Installation script for QGIS Geemap Plugin
#
# Usage:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --name, -n NAME   Plugin folder name (default: qgis_geemap)"
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    echo "Please manually copy the qgis_geemap folder to your QGIS plugins directory."
    exit 1
fi
...
if [[ "$REMOVE" == true ]]; then
    ...
    echo "  3. Enable 'QGIS Geemap'"
    echo ""
    echo "Requirements:"
    echo "  - earthengine-api: pip install earthengine-api"
    echo "  - geemap (optional): pip install geemap"
    echo ""
fi
