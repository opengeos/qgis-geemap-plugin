#!/usr/bin/env python3
"""
Package the QGIS Geemap plugin for upload to the official QGIS plugin repository.

This script creates a zip file with the following characteristics:
- Root folder renamed from 'qgis_geemap' to the target plugin name
...
def package_plugin(
    ...
    Args:
        source_dir: Path to the qgis_geemap directory
        output_path: Optional path for the output zip file
        target_name: Name for the root folder in the zip (default: 'qgis_geemap')
...
def main():
    parser = argparse.ArgumentParser(
        description="Package QGIS Geemap plugin for repository upload",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output path for the zip file (default: qgis_geemap-{version}.zip in the same folder as this script)",
    )
    parser.add_argument(
        "--source",
        "-s",
        type=Path,
        default=None,
        help="Source directory (default: qgis_geemap in the same folder as this script)",
    )
    parser.add_argument(
        "--name",
        "-n",
        type=str,
        default="qgis_geemap",
        help="Target plugin name for the zip root folder (default: qgis_geemap)",
    )
    parser.add_argument(
        "--no-version",
        action="store_true",
    ...
    source_dir = args.source if args.source else script_dir / "qgis_geemap"
