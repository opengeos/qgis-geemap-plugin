#!/usr/bin/env python3
"""
Cross-platform installation script for QGIS Geemap Plugin.
...
def install_plugin(
    source_dir: Path, plugin_dir: Path, plugin_name: str = "qgis_geemap"
) -> bool:
...
def remove_plugin(plugin_dir: Path, plugin_name: str = "qgis_geemap") -> bool:
...
def main():
    parser = argparse.ArgumentParser(description="Install or remove QGIS Geemap Plugin")
    parser.add_argument(
        "--remove",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="qgis_geemap",
        help="Plugin folder name in QGIS plugins directory (default: qgis_geemap)",
    )
    args = parser.parse_args()

    # Get script directory
    script_dir = Path(__file__).parent.resolve()
    source_dir = script_dir / "qgis_geemap"
    ...
    if args.remove:
        ...
        if success:
            ...
            print(f"  3. Enable 'QGIS Geemap'")
            print()
            print("Requirements:")
            print("  - earthengine-api: pip install earthengine-api")
            print("  - geemap (optional): pip install geemap")
            print()
